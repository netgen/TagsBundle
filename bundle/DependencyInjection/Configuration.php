    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $this->generateScopeBaseNode($rootNode)
            ...
            ->arrayNode('sort')
                ...
                ->children()
                    ->enumNode('by')
                        ->info('Set the default property by which all child tags will be sorted by')
                        ->values(['id', 'keyword', 'modified', 'priority'])
                        ->defaultValue('id')
                    ->end()
                    ->enumNode('order')
                        ->info('Set the default sorting order by which all child tags will be ordered by')
                        ->values(['asc', 'desc'])
                        ->defaultValue('asc')
                    ->end()
                    ->arrayNode('root')
                        ->addDefaultsIfNotSet()
                        ->info('Set the default property and order by which the root tag children will be sorted')
                        ->children()
                            ->enumNode('by')
                                ->info('Set the default property by which root tag children will be sorted by')
                                ->values(['id', 'keyword', 'modified', 'priority'])
                                ->defaultValue('id')
                            ->end()
                            ->enumNode('order')
                                ->info('Set the default sorting order by which root tag children will be ordered by')
                                ->values(['asc', 'desc'])
                                ->defaultValue('asc')
                            ->end()
                        ->end()
